<template>
	<div>
		...
		<scroll-div scroll-y class="indexes" :scroll-into-div="'indexes-'+ listCurID" :style="[{height:'calc(100vh - '+ CustomBar + 'px - 50px)'}]"
				...
				<div :class="'indexItem-' + item.name" :id="'indexes-' + item.name" :data-index="item.name">
					...
					<div class="cu-list menu-avatar no-padding">
						<div class="cu-item" v-for="(items,sub) in 2" :key="sub">
							...
							<div class="content">
								<div class="text-grey">{{item.name}}<text class="text-abc">{{list[sub].name}}</text>君</div>
...
<style>
	...
	movable-div.indexBar-item::before {
		content: "";
		display: block;
		position: absolute;
		left: 0;
